...
def HostelAdd(master):
    Hostel_Registration = Toplevel(master)
    Hostel_Registration.geometry("600x300+450+190")
    
    def on_start():
        TxtNo.delete(0, END)
        TxtHostelName.delete(0, END)
        
        n1 = Max_No('gate_pass_hostel_master', 'hid', clg_id)
        TxtNo.insert(0, n1)

        List_Data.delete(*List_Data.get_children())
        sql1 = "SELECT hid, hname, id FROM gate_pass_hostel_master WHERE college_id = %s"
        ...
        for i in data1:
            ...
            n2 = i[1]
            n3 = str(i[2])
            List_Data.insert("", 'end', text=n1, values=(n1, n2, n3))
    ...
    def Save_Data():
        ...
        if n1 != '' and n2 != '' and BtnSave['text'] == 'Save':
            sql = "INSERT INTO gate_pass_hostel_master(hid, hname, college_id) values(%s, %s, %s)"
            db_cursor.execute(sql, (n1, n2, clg_id))
            db_connection.commit()
            messagebox.showinfo("information","Successful...",parent=Hostel_Registration)
        
        elif n1 != '' and n2 != '' and BtnSave['text'] == 'Update':
            sql = "UPDATE gate_pass_hostel_master SET hid = %s, hname = %s WHERE id = %s and college_id = %s"
            db_cursor.execute(sql, (n1, n2, n3, clg_id))
            db_connection.commit()
            messagebox.showinfo("information","Successful...",parent=Hostel_Registration)
            BtnSave['text'] = 'Save'
        else:
            messagebox.showerror("Error","Please fill all fields...",parent=Hostel_Registration)
    ...
    def expand():
        Hostel_Registration.geometry("1000x300+300+190")
        BtnShow['text'] = 'Reset'
    ...
    Frm1 = LabelFrame(Hostel_Registration, height=285, width=580, bg='#518aed')
    Frm1.place(x=10, y=10)

    LblHead = Label(Frm1, text='Hostel Registration', font=('Arial',25), bg='#518aed', fg='white')
    ...
    LblHostelName = Label(Frm1, text='Hostel Name', font=('Arial',17), bg='#518aed', fg='white',)
    ...
    columns = ('#1', '#2')
    List_Data = ttk.Treeview(Hostel_Registration,show="headings",height="10", columns=columns)
    List_Data.heading('#1', text='Hostel No', anchor='center')
    List_Data.column('#1', width=80, anchor='center', stretch=False)
    List_Data.heading('#2', text='Hostel Name', anchor='center')
    List_Data.column('#2', width=150, anchor='w', stretch=True)
    List_Data.place(x=610, y=10, height=280, width=370)  

    vsb= ttk.Scrollbar(Hostel_Registration, orient=tk.VERTICAL,command=List_Data.yview)  
