...
def Daily_Report(master):
    ...
    def Show_Report():
        ...
        pdf.cell(80)
        pdf.cell(500, 5, txt="Date : "+head3, align='L')
        ...
        sql1 = f"SELECT gate_pass_account_master.ac_name, gate_pass_account_master.ac_contact, gate_pass_master.reason, gate_pass_master.pass_status FROM gate_pass_master INNER JOIN gate_pass_account_master on gate_pass_account_master.std_id = gate_pass_master.std_id WHERE gate_pass_master.date_out = %s"
        db_cursor.execute(sql1,(dt1,))
        ...
        if db_cursor.rowcount > 0:
            ...
            pdf.output("D:\Ishan\HostelManagement\Reports\Daily_Report.pdf",'F')
    
            os.startfile("D:\Ishan\HostelManagement\Reports\Daily_Report.pdf")
    ...
    Lblhead = Label(Frm1, text="Daily Report", font=('Times New Roman', 22), fg='red')
    Lblhead.place(x=110, y=10)
    
    LblDate1 = Label(Frm1, text='Date', font=('Times New Roman', 18))
    LblDate1.place(x=80, y=70)
    TxtDate1 = DateEntry(Frm1, font=('Times New Roman', 18), date_pattern = 'dd/mm/yyyy', width=10)
    TxtDate1.place(x=190, y=70)
